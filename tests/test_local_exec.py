...
from jax_solitons.campaign import run_multi, split_configs
...
def test_inprocess_runs_each_config(tmp_path, monkeypatch):
    ...
    cfgs = [RunConfig("faddeev_cp1", N=8, L=8.0, params={"R": 2.0}),
            RunConfig("faddeev_cp1", N=8, L=8.0, params={"R": 3.0})]
    recs = ex.run(cfgs)
    assert len(recs) == 2
...
def test_inprocess_participates_in_run_multi(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(remote, "load_run_fn", lambda ref: _trivial)
    ex = InProcessExecutor("x:y", work_dir=str(tmp_path))
    cfgs = [RunConfig("faddeev_cp1", N=8, L=8.0, params={"R": float(i)})
            for i in range(3)]
