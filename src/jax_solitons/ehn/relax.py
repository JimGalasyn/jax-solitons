...
STEP-SIZE BOUND — α < 2/H, AND H IS NOT dx-INDEPENDENT BELOW dx≈0.1.
...
**If you refine the grid below dx ≈ 0.1, α MUST come down as 1/dx².** Holding
α fixed while shrinking dx diverges: at dx=0.05 the limit is 3.3e-5, so the
default 1e-4 is already 3x over. The crossover is where U/dx² starts to rival
8λ, so it moves with U and λ — recompute rather than trusting this table if
either changes.
...
That also means Which means EHN's published α=4e-4 exceeds the
bound of this functional by 1.6x. Their d³-weighted energy (their Eq.1 is
...
if __name__ == "__main__":
    ...
    ap.add_argument("--alpha", type=float, default=1e-4,
                    help="descent step. STABILITY: alpha < 2/H. H = 8*lam "
                         "(=8000 at lam=1000) for dx >= 0.2, but grows as "
                         "1/dx^2 below dx~0.1 -- at dx=0.05 the limit is "
                         "3.3e-5 and this default already diverges. Scale "
                         "alpha down as 1/dx^2 when refining the grid.")
